...
### 6.1. Совместимый фундамент для следующих задач

Эта задача остаётся самостоятельной исходной RBAC-задачей и не реализует search или Redis. Но её результат является контрактом для задач №3–4:

- роли остаются `OWNER`, `MEMBER`, `VIEWER`; task3/task4 не добавляют `ADMIN` и не меняют permission matrix;
- `OWNER`, `MEMBER` и `VIEWER` имеют `view` для project/document, поэтому все трое смогут использовать будущий workspace search и кешируемые read endpoints;
- outsider получает hidden `404` до выполнения resource query; будущие search/cache layers не могут заменить или отложить policy;
- `GET /workspaces/:workspaceId/projects` является единственным tenant-specific project-list route, который task4 сможет кешировать без изменения URL;
- будущий search route имеет отдельный контракт `GET /workspaces/:workspaceId/documents/search`; task2 не добавляет его и не меняет существующие document routes;
- списки ограничиваются tenant на уровне Prisma, а будущий search обязан ограничивать `Project.workspaceId` внутри SQL до ranking и pagination;
- feature-service вызывает `AccessPolicyService`, затем выполняет business read/write; будущий cache располагается после policy и перед существующим DB loader;
- policy context для `requireProject`/`requireDocument` должен позволять безопасно определить `workspaceId` после успешной проверки: task4 использует его для invalidation после project/document mutation;
- обычный PATCH не принимает `status`, archive остаётся отдельным idempotent endpoint; task4 инвалидирует cache, но не меняет state semantics;
- public response shapes из этого раздела остаются стабильными на cache hit и miss; cache не является поводом удалить `_count.documents`, `author.name`, `projectId`, `title` или `content` из тех routes, где они уже публичны;
- migrations задачи №2 остаются неизменяемой историей; task3 добавляет только `pg_trgm`/index migrations, а task4 не переписывает schema history.

Membership/role mutation endpoints не входят в задачу №2. Если они появятся позднее, они обязаны использовать тот же hidden-404 policy и инвалидировать зависящие workspace reads; task4 не должна выдумывать отсутствующий route.
